...
WHERE service_date >= DATE_FORMAT(
  ...
  AND BINARY p_team LIKE '%Industrial%'
...
WHERE LOWER(ci.p_team) LIKE '%industrial%'
...
FROM csa_finance_invoiced
WHERE LOWER(p_team) LIKE '%industrial%'
...
WHERE a.account_status = 1
  AND LOWER(a.p_team) LIKE '%industrial%'
  AND (
        LOWER(sp.p_team) LIKE '%industrial%'
        OR sp.p_team IS NULL
      )
  AND (
        LOWER(p.p_team) LIKE '%industrial%'
        OR p.p_team IS NULL
      )
...
//C:\Users\shrey\Desktop\raeanaylytics\raeAnalytics\dashboard-backend\queries\industrialQueries.js
