...
WHERE service_date >= DATE_FORMAT(
  ...
  AND BINARY p_team LIKE '%Building%'
...
WHERE LOWER(ci.p_team) LIKE '%building%'
...
FROM csa_finance_invoiced
WHERE LOWER(p_team) LIKE '%building%'
...
SELECT
    ...
    COUNT(DISTINCT combined.project_id) AS total_projects_worked,
    COUNT(DISTINCT combined.subproject_id) AS total_subprojects_worked
...
LEFT JOIN (
    ...
    SELECT 
        p.project_managers_id AS user_id,
        p.project_id,
        NULL AS subproject_id
    FROM projects p
    WHERE LOWER(p.p_team) LIKE '%building%'
    ...
    -- Subproject assignments (project counted via parent)
    SELECT 
        sp.project_managers_id,
        sp.project_id,
        sp.table_id
    FROM subprojects sp
    JOIN projects p ON p.project_id = sp.project_id
    WHERE LOWER(sp.p_team) LIKE '%building%'
      AND LOWER(p.p_team) LIKE '%building%'
...
) AS combined ON combined.user_id = a.user_id

WHERE a.account_status = 1
  AND LOWER(a.p_team) LIKE '%building%'
...
//C:\Users\shrey\Desktop\raeanaylytics\raeAnalytics\dashboard-backend\queries\BuildingQueries.js
